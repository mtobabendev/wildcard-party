...
import { database, databaseNotConfigured } from './database.js'
import { ensureSocialSchema, ownerHash } from './social-db.js'
...
export async function updateAccountProfile(accountId, { handle, displayName, bio }) {
  ...
  try {
    const [accountRows] = await sql.transaction([
      sql`
...
export async function claimAnonymousContent(account, ownerToken) {
  ...
  const [posts, comments] = await sql.transaction([
    sql`
      UPDATE wildcard_posts
      SET
