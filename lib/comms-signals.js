...
async function sqlForInsert({
  ...
  return sql`
    ...
    WHERE call.id = ${callId}::uuid
      ...
      AND (
        call.caller_account_id = ${senderAccountId}::uuid
        OR call.callee_account_id = ${senderAccountId}::uuid
      )
      AND (
        (
          ${type} = 'offer'
          AND call.caller_account_id = ${senderAccountId}::uuid
        )
        OR (
          ${type} = 'answer'
          AND call.callee_account_id = ${senderAccountId}::uuid
          AND EXISTS (
            SELECT 1
            FROM wildcard_call_signals offer
            WHERE offer.call_id = call.id
              AND offer.signal_type = 'offer'
          )
        )
        OR (
          ${type} = 'ice'
          AND (
            ...
            OR (
              call.callee_account_id = ${senderAccountId}::uuid
              AND EXISTS (
                SELECT 1
                FROM wildcard_call_signals answer
                WHERE answer.call_id = call.id
                  AND answer.signal_type = 'answer'
              )
            )
          )
        )
