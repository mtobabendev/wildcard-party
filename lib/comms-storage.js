...
export function isAudioAttachmentType(value) {
  return normalizedAttachmentType(value).startsWith('audio/')
}

export function isVideoAttachmentType(value) {
  return normalizedAttachmentType(value).startsWith('video/')
}
...
export function isInlineImageType(value) {
  return INLINE_IMAGE_TYPES.has(normalizedAttachmentType(value))
}

export function isInlineMediaType(value) {
  return (
    isInlineImageType(value) ||
    isAudioAttachmentType(value) ||
    isVideoAttachmentType(value)
  )
}
...
function attachmentNameFromKey({
  key,
  conversationId,
  accountId,
}) {
  ...
  if (objectName.length < 38 || objectName[36] !== '-') return ''
  ...
  const safeName = sanitizeAttachmentName(objectName.slice(37))
  return safeName === 'attachment' && !objectName.slice(37).trim()
    ? ''
    : safeName
}
...
export function attachmentKeyBelongsTo({
  ...
  return Boolean(attachmentNameFromKey({
    key,
    conversationId,
    accountId,
  }))
...
export async function verifyUploadedAttachment({
  ...
  const keyName = attachmentNameFromKey({
    key,
    conversationId,
    accountId,
  })
  ...
  return {
    ...
    isAudio: isAudioAttachmentType(contentType),
    isVideo: isVideoAttachmentType(contentType),
  }
}

function responseDisposition(name, inline) {
...
export async function createAttachmentDownload({
  ...
  const { client, bucket } = clientAndBucket()
  const inline = isInlineMediaType(contentType)

  const command = new GetObjectCommand({
    Bucket: bucket,
    Key: storageKey,
    ResponseContentDisposition: responseDisposition(name, inline),
