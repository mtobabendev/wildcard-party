  ...
  const rows = await sql.unsafe(
    `${callSelect()}
     WHERE call.id = $1::uuid
       AND (
         call.caller_account_id = $2::uuid
         OR call.callee_account_id = $2::uuid
       )
     LIMIT 1`,
...
async function selectCallByClientId(accountId, clientCallId) {
  ...
  const rows = await sql.unsafe(
    `${callSelect()}
     WHERE call.caller_account_id = $1::uuid
       AND call.client_call_id = $2::uuid
     LIMIT 1`,
    [currentId, currentClientCallId],
  )
...
export async function getActiveCallForAccount(accountId) {
  ...
  const rows = await sql.unsafe(
    `${callSelect()}
     WHERE (
       call.caller_account_id = $1::uuid
       OR call.callee_account_id = $1::uuid
     )
